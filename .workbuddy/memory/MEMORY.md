...
- **方向A（已落地, 提交 598d77a）：编译 `luci-app-adguardhome` + i18n 作 Web 壳，引擎仍走二进制注入。**
  - 实测依赖（25.12）：`luci-app-adguardhome` `LUCI_DEPENDS:=+adguardhome` + `LUCI_EXTRA_DEPENDS:=adguardhome (>=0.107.73-r3)` —— **硬依赖引擎包**。若放开 luci-app 而注释 adguardhome 引擎包，编译会因 unmet dependency 失败。
  ...
  - 版本范围：项目仅支持 **25.12**（已从仓库移除 24.10 相关 config/feeds/菜单）。故 25.12 的 full/mini 放开 luci-app+i18n；build.sh 去依赖 sed 仅覆盖 25.12。
...
- GitHub Actions workflow 在 Actions 页面"不显示/不可触发"的两类元凶：① 远端 `.yml` 含未解决的 `<<<<<<<` 合并冲突标记（YAML 报错→GitHub 静默忽略整文件）；② 误用非 `.yml/.yaml` 扩展名（如 `.ymb`）→GitHub 不加载。本地改完须 `git push` 到默认分支才生效。校验用 `actionlint`：`workflow_dispatch` 的 choice `options` 须为字符串数组；`softprops/action-gh-release@v2` 无 `overwrite_files` input。
